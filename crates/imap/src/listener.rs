...
impl ImapListener {
    pub async fn bind(addr: SocketAddr) -> Result<Self> {
        let listener = TcpListener::bind(addr).await.map_err(|err| {
            Error::internal(format!("could not bind the IMAP listener on {addr}: {err}"))
        })?;
        Ok(Self { listener })
    }
        ...
        Fut: Future<Output = ()> + Send + 'static,
    {
        loop {
            let (stream, peer) = self
                ...
                .accept()
                .await
                .map_err(|err| Error::internal(format!("IMAP accept failed: {err}")))?;
...
pub async fn register_imap(
    ...
    if let Some(addr) = starttls {
        let listener = ImapListener::bind(addr).await?;
        register_imap_plain(
            registry,
            listener,
            acceptor.clone(),
    ...
    if let Some(addr) = implicit {
        let listener = ImapListener::bind(addr).await?;
        register_imap_implicit(registry, listener, acceptor, directory, blobs, notifier);
        registered += 1;
    }
...
pub fn register_imap_plain(
    ...
    registry.register_listener("imap:143", move || async move {
        let result = listener
            .serve(move |stream, peer| {
                ...
                async move {
                    ...
                    {
                        tracing::debug!(%peer, error = %err, "IMAP connection ended");
                    }
                }
            })
            .await;
        if let Err(err) = result {
            tracing::error!(error = %err, "IMAP 143 listener stopped");
        }
    });
}

pub fn register_imap_implicit(
    registry: &Registry,
    listener: ImapListener,
    acceptor: TlsAcceptor,
    directory: Directory,
    ...
    let acceptor = Arc::new(acceptor);
    registry.register_listener("imap:993", move || async move {
        let result = listener
            .serve(move |stream, peer| {
                let acceptor = acceptor.clone();
                let directory = directory.clone();
            ...
            .await;
        if let Err(err) = result {
            tracing::error!(error = %err, "IMAP 993 listener stopped");
        }
    });
}
...
async fn handle_implicit(
    ...
    let secured = acceptor
        .accept(stream)
        .await
        .map_err(|err| Error::protocol(format!("IMAP implicit TLS handshake failed: {err}")))?;
    if directory.ip_rules().blocks(peer.ip()) {
        use tokio::io::AsyncWriteExt;
...
mod tests {
    ...
    use irixmail_store::{ChangeNotifier, FsBlobStore, RocksdbStore, Store};
    ...
    fn blob_store(dir: &TempDir) -> Arc<dyn BlobStore> {
    ...
    async fn an_accepted_connection_reaches_the_handler() {
        let listener = ImapListener::bind(loopback()).await.unwrap();
    ...
    async fn both_ports_register_two_listeners() {
        ...
        let names: Vec<String> = registry
            ...
            .map(|(name, _)| name)
            .collect();
        assert_eq!(names, vec!["imap:143", "imap:993"]);
    }

    #[tokio::test]
    async fn the_plaintext_listener_greets_and_logs_out() {
        let registry = Registry::new();
        let dir = TempDir::new();
        let listener = ImapListener::bind(loopback()).await.unwrap();
        let addr = listener.local_addr().unwrap();
        ...
        let mut farewell = [0u8; 512];
        let read = client.read(&mut farewell).await.unwrap();
        assert!(farewell[..read].starts_with(b"* BYE"));

        tasks.abort_all();
    }

    #[tokio::test]
    async fn a_blocked_ip_is_told_bye_instead_of_the_greeting() {
        ...
        assert!(text.starts_with("* BYE access denied"), "got: {text:?}");
    ...
    #[tokio::test]
    async fn gate2_app_password_login_select_append_fetch_store_search_expunge() {
        ...
        let server_name = ServerName::try_from("localhost").unwrap();
        let mut client = connector().connect(server_name, stream).await.unwrap();
        let _ = read_until(&mut client, "* OK").await;

        let login = format!("a LOGIN alice@example.com {}\r\n", minted.plaintext);
        let out = exchange(&mut client, login.as_bytes(), "a OK").await;
        assert!(out.contains("a OK"), "app-password LOGIN: {out}");

        let out = exchange(&mut client, b"b SELECT INBOX\r\n", "b OK").await;
        assert!(out.contains("b OK"), "SELECT INBOX: {out}");

        let message = "Subject: Gate2\r\nFrom: bob@example.net\r\n\r\nhello gate two\r\n";
        let append = format!("c APPEND INBOX {{{}+}}\r\n{}\r\n", message.len(), message);
        let out = exchange(&mut client, append.as_bytes(), "c OK").await;
        assert!(out.contains("c OK"), "APPEND completes: {out}");

        let out = exchange(&mut client, b"d FETCH 1 (BODY[] FLAGS)\r\n", "d OK").await;
        assert!(
            out.contains("Subject: Gate2"),
            "FETCH returns appended headers: {out}"
        );
        assert!(
            out.contains("hello gate two"),
            "FETCH returns appended body: {out}"
        );

        let out = exchange(&mut client, b"e STORE 1 +FLAGS (\\Seen)\r\n", "e OK").await;
        assert!(out.contains("\\Seen"), "STORE echoes the new flag: {out}");

        let out = exchange(&mut client, b"f SEARCH SEEN\r\n", "f OK").await;
        assert!(
            out.contains("* SEARCH 1"),
            "SEARCH SEEN finds the message: {out}"
        );

        let _ = exchange(&mut client, b"g STORE 1 +FLAGS (\\Deleted)\r\n", "g OK").await;
        let out = exchange(&mut client, b"h EXPUNGE\r\n", "h OK").await;
        assert!(
            out.contains("* 1 EXPUNGE"),
            "EXPUNGE removes the message: {out}"
        );

        let out = exchange(&mut client, b"i SEARCH ALL\r\n", "i OK").await;
        assert!(
            out.contains("* SEARCH\r\n"),
            "mailbox is empty after expunge: {out}"
        );

        let _ = exchange(&mut client, b"x LOGOUT\r\n", "x OK").await;
        tasks.abort_all();
    }

    #[tokio::test]
    async fn idle_pushes_exists_for_new_mail_and_noop_check_flush_pending() {
        use irixmail_directory::{app_password, Role};
        use irixmail_mail::{allocate_document_id, deliver, provision_mailboxes, DeliveryRequest};
        ...
        let account = directory
            ...
            .create("alice", domain.id, "Alice", Role::User)
            .unwrap();
        let minted = app_password::generate(1, "idle", 0).unwrap();
        directory
            .credentials()
            .add_app_password(account.id, minted.record)
            .unwrap();

        let blobs = blob_store(&dir);
        ...
        let deliver_one = |subject: &str| {
            let raw = format!(
                "From: bob@example.net\r\nTo: alice@example.com\r\nSubject: {subject}\r\n\r\nnew mail\r\n"
            );
            let document_id = allocate_document_id(store.as_ref(), account.id as u32).unwrap();
            deliver(
                store.as_ref(),
                blobs.as_ref(),
                &notifier,
        ...
        let login = format!("a LOGIN alice@example.com {}\r\n", minted.plaintext);
        let _ = exchange(&mut client, login.as_bytes(), "a OK").await;
        let out = exchange(&mut client, b"b SELECT INBOX\r\n", "b OK").await;
        assert!(
            out.contains("* 0 EXISTS"),
            "SELECT reports an empty inbox: {out}"
        );

        let out = exchange(&mut client, b"c IDLE\r\n", "+ idling").await;
        assert!(
            out.contains("+ idling"),
            "IDLE answers the continuation: {out}"
        );

        deliver_one("first");
        let out = read_until(&mut client, "* 1 EXISTS").await;
        assert!(
            out.contains("* 1 EXISTS"),
            "IDLE pushes the new-mail EXISTS: {out}"
        );

        let out = exchange(&mut client, b"DONE\r\n", "c OK").await;
        assert!(
            out.contains("c OK IDLE completed"),
            "DONE ends the idle: {out}"
        );

        deliver_one("second");
        let out = exchange(&mut client, b"d NOOP\r\n", "d OK").await;
        assert!(
            out.contains("* 2 EXISTS"),
            "NOOP flushes the pending EXISTS: {out}"
        );

        deliver_one("third");
        let out = exchange(&mut client, b"e CHECK\r\n", "e OK").await;
        assert!(
            out.contains("* 3 EXISTS"),
            "CHECK flushes the pending EXISTS: {out}"
        );
