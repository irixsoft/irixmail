...
pub struct ImplicitTlsListener {
    listener: TcpListener,
}

impl ImplicitTlsListener {
    pub async fn bind(addr: SocketAddr) -> Result<Self> {
        let listener = TcpListener::bind(addr).await.map_err(|err| {
            Error::internal(format!(
                "could not bind the implicit TLS submission listener on {addr}: {err}"
            ))
        })?;
        Ok(Self { listener })
    }
        ...
        Fut: Future<Output = ()> + Send + 'static,
    {
        loop {
            let (stream, peer) = self.listener.accept().await.map_err(|err| {
                Error::internal(format!("implicit TLS submission accept failed: {err}"))
            })?;
...
pub fn register_submission_465(
    registry: &Registry,
    listener: ImplicitTlsListener,
    acceptor: TlsAcceptor,
    services: SubmissionServices,
) {
    let acceptor = Arc::new(acceptor);
    registry.register_listener("smtp:465", move || async move {
        let result = listener
            .serve(move |stream, peer| {
                ...
                async move {
                    if let Err(err) = handle_connection(stream, peer, &acceptor, services).await {
                        tracing::debug!(%peer, error = %err, "implicit TLS submission connection ended");
                    }
                }
            })
            .await;
        if let Err(err) = result {
            tracing::error!(error = %err, "implicit TLS submission listener stopped");
        }
    });
}
...
async fn handle_connection(
    ...
    let secured = acceptor.accept(stream).await.map_err(|err| {
        Error::protocol(format!("implicit TLS submission handshake failed: {err}"))
    })?;
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "irixmail-smtp-listener-sub465-{}-{unique}",
                std::process::id()
            ));
